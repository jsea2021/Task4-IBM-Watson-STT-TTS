...
#Setup Service
API ='a-fJ9EnVkEQ1M0OCcy7H8oKu2rKBHOZqKKYijTRnl-Jz'
URL = 'https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/c81f341b-cfdd-49a6-9484-e985d5fb2dac'
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        print(output_text)
        #STT
        with open ('TextOutput.txt','w') as text:
...
def main():
    ...
    ws.run_forever()
     #TTS 
    with open ('TextOutput.txt', 'r') as f:
       tts_Speech = f.readlines()
       tts_Speech = [line.replace('\n','') for line in tts_Speech]
       tts_Speech = ''.join(str(line) for line in tts_Speech)

    with open ('TextToSpeech.mp3','wb') as a:
       res =tts.synthesize(tts_Speech,accept='audio/mp3',voice='en-US_AllisonV3Voice').get_result()
       a.write(res.content)    
       a.close()
    print(tts_Speech)
